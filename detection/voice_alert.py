...
STATIC_IMAGE_PATH = os.path.join(BASE_DIR, "static", "Voice_Alert_Images")
os.makedirs(STATIC_IMAGE_PATH, exist_ok=True)

def detect_and_prepare_voice_alert():
    ...
    filename = f"voice_alert_image_{timestamp}.jpg"
    full_path = os.path.join(STATIC_IMAGE_PATH, filename)
    web_path = f"/static/Voice_Alert_Images/{filename}"
    ...
    if result in ["Mask", "No Mask"]:  # Log only valid detections
        log_compliance(result, detection_type="Voice Alert")
